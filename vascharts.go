...
func (g *game) Setupcharts() {
	...
	for c := 0; c < chartnum; c++ {
		for i := 0; i < datapointsmax; i++ {
			g.chart[c].drawonechart(g.chartSize)
			...
			g.chart[c].mLine[i].StrokeColor = color.Black
			if i < g.datapoints {
...
func (g *game) Updatecharts() error {
	if g.countrunning() == 0 || g.endmeasuring {
		return nil
	}
	if g.chart == nil {
		g.Setupcharts()
	...
	for c := 0; c < chartnum; c++ {
		...
		if newdata > -1 {
			...
			g.chart[c].mValue[g.chart[c].gotpoints-1] = newdata
			if g.averagepoints > 1 {
				avpoints := g.averagepoints
				if avpoints < 1 {
					avpoints = 1
				}
				if g.chart[c].gotpoints < avpoints {
					avpoints = g.chart[c].gotpoints
				}
				var avsum float32 = 0
				for a1 := 0; a1 < avpoints; a1++ {
					avsum += g.chart[c].mValue[a1]
				}
				newdata = avsum / float32(avpoints)
			}
			if newdata > g.chart[c].mMax {
				g.chart[c].mMax = fixmax(newdata)
				g.chart[c].drawoneyaxistexts()
				// recalculate all T-axis values based on the change to max!!!!
				for i := 0; i < g.chart[c].gotpoints; i++ {
					g.chart[c].mY[i] = g.chart[c].mValue[i] / (g.chart[c].mMax + 1)
				}
			} else {
				// adjust for Y-size 0 ... 1 just the new Y-value
				g.chart[c].mY[g.chart[c].gotpoints-1] = newdata / (g.chart[c].mMax + 1)
			}
			// find new max when oldMax is dissapearing...
			if fixmax(g.chart[c].mValue[0]) == g.chart[c].mMax {
				var newmax float32 = 0
				for i := 1; i < g.chart[c].gotpoints; i++ {
					if newmax < g.chart[c].mValue[i] {
						newmax = g.chart[c].mValue[i]
					}
				}
				g.chart[c].mMax = fixmax(newmax)
				g.chart[c].drawoneyaxistexts()
				for i := 1; i < g.chart[c].gotpoints; i++ {
					g.chart[c].mY[i] = g.chart[c].mValue[i] / (g.chart[c].mMax + 1)
				}
			...
			// time...
			g.chart[c].mTime[g.chart[c].gotpoints-1] = time.Now().Format(time.RFC3339)[11:16]
			// move that line...
			...
			for i := 1; i < g.chart[c].gotpoints; i++ {
				...
				if g.chart[c].gotpoints == g.datapoints {
					...
					g.chart[c].mValue[i-1] = g.chart[c].mValue[i]
					g.chart[c].mTime[i-1] = g.chart[c].mTime[i]
				}
			}
		}
	}
	return g.Redrawcharts()
}
func (g *game) Redrawcharts() error {
	if g.countrunning() == 0 {
		return errors.New("#1 Redrawchars: not running")
	}
	if g.chart == nil {
		g.Setupcharts()
	}
	for c := 0; c < chartnum; c++ {
		if g.chart[c].gotpoints > 1 {
			...
			g.chart[c].drawonechartlines(g.chart[c].gotpoints)
			g.chart[c].drawonexaxistexts(g.chart[c].mTime)
			g.chart[c].refreshonechart(c)
		}
	}
	g.cont.Refresh()
	return nil
...
func (lc *LineChart) drawonexaxistexts(t [datapointsmax]string) {
