...
func (db *dbtype) Setupdb(g *game) error {
	var err error
	var temp fyne.URI
	temp, err = storage.Child(g.getdocumentpath(), g.app.UniqueID()+".db")
	...
	if _, err = os.Stat(db.Databasepath); err == nil {
		...
		log.Println("#2 vas.db not found, creating new db: " + db.Databasepath)
		var file *os.File
		...
		if err != nil {
			...
			log.Println("vas.db tables created")
		}
		err = db.opendb()
		if err != nil {
